...
name→`<@id>` step, so the bot emits raw display names (the `<@Erin Evans>` bug).
...
- **3.2** Status: use `assistant.threads.setStatus` for "thinking/working" indicators instead of
  ...
  **company-branded emoji** + short phrases (e.g. `:acmedata: crunching the data…`).
...
1. **Branded emoji** (you): replace `config.status_loading_messages` placeholders with real acmedata custom-emoji names.
