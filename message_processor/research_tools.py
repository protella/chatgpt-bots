...
def get_start_background_job_schema() -> dict:
    ...
    return {
        ...
        "description": (
            ...
            "both — and any file or computation you would otherwise build in the inline sandbox "
            "that will take minutes rather than seconds. It works "
            ...
            "A `research` phase cannot fetch or search Slack history or the workspace; it sees "
            "only the conversation snapshot captured at dispatch, not the full record. For a "
            "request to summarize, analyze, or count Slack conversation content over a period, "
            "use `export_conversation` and compute over the export instead. If a requested file "
            "makes a build phase appropriate, dispatch it with `mode=build` and tell that job to "
            "call `export_conversation` in its own sandbox; do not pass it an export path from "
            "this turn.\n\n"
            "Pick the mode:\n"
