...
* **There is no `canvases.read`.** Content comes back by downloading `url_private`, and it
  ...
  converter; `canvas_content.html_to_markdown` is it.
...
#
# The converter and its login-page marker live in `canvas_content` now: the document path reads
# canvases too (`document_handler.parse_canvas`), and one converter is the only way both readers
# see the same canvas. Re-exported under the private names this module has always used.
_CANVAS_MARKER = CANVAS_MARKER
