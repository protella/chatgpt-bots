...
Without this the bot dead-ends on "what's happening in #product-insights?" asked from a DM: the
...
def _normalize(raw: str) -> str:
    """"#Product-Insights " → "product-insights". Slack stores names lowercased; the model (and the
...
class SlackChannelLookupToolMixin:
    ...
    def get_lookup_channel_tool_schema(self) -> Dict[str, Any]:
        return {
            ...
            "name": "lookup_channel",
            "description": (
                "Resolve a Slack channel NAME (\"#product-insights\" or \"product-insights\") to the "
    ...
    async def execute_lookup_channel(self, ctx: Any, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not want:
            return {"ok": False, "error": "bad_arguments",
                    "message": "Give me the channel name, e.g. #product-insights."}
