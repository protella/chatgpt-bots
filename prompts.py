...
- "placement": "thread" or "channel". THREAD IS THE DEFAULT — threads keep the channel scannable and keep follow-ups attached to their question, so when in doubt, thread. Pick "channel" only when ALL of these hold: the answer is genuinely one short line, the whole room benefits from seeing it inline, and no back-and-forth is likely. Always prefer "thread" when the triggering message addressed multiple parties or another assistant is likely to answer too — everyone's replies then collect under the message instead of scattering the channel. Channel placement only takes effect where the channel has opted into top-level replies; elsewhere it is coerced to thread — that is expected, don't fight it.
