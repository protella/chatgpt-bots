...
- "memory_op": the durable record to make — it applies to CHANNEL-scope feedback ONLY, operating on channel memory: "add" a new preference, "update:<id>" an existing preference (the channel-memory facts you were given show their [#id]), "delete:<id>" to REVERSE a recorded preference ("you can react again" → delete the matching one), or "none". For a THREAD-scoped verdict ALWAYS use "none": a thread-scoped "stop replying here" is acknowledged for this message only and nothing is stored — there is no persistent per-thread mute.
- "structural_request": set to "participation" ("only reply when I tag you", "be more active in this channel"), "placement" ("keep your replies in threads", "you can reply in the channel"), or "both" — but ONLY when the person gives an explicit, direct instruction to change the CHANNEL'S settings in this message. This routes the message to the assistant so it changes the setting itself with judgment; you never change settings. Use "none" for soft preferences and momentary asides — a vague "you're a bit chatty" is a standing preference to remember (memory_op), NOT a structural request.
