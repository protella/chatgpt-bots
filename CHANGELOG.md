...
- **Dependabot `versioning-strategy: lockfile-only`** — bot updates the lockfile but leaves the manifest alone; minor+patch grouping now works properly
...
- **`make install`** now uses `--require-hashes` against the lockfile for reproducible + supply-chain-safe installs
