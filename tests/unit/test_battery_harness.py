...
def test_ack_addressee_names_are_address_not_content():
    """MEASURED IN: a real trial wrote "Thanks, Vinay." and the grammar called the name free
    ...
    assert bh.origin_ack_violation("Thanks, Vinay.", addressees=("Vinay Raj",)) is None
    assert bh.origin_ack_violation("Thanks, Vinay Raj — got it.",
                                   addressees=("Vinay Raj",)) is None
    assert bh.origin_ack_violation("Thanks, Vinay.") is not None
    # A name is address only — it cannot vouch for the free prose beside it.
    assert bh.origin_ack_violation("Vinay handled it.", addressees=("Vinay Raj",)) is not None
    ...
    assert bh.origin_ack_violation("Thanks for ChatGPT closing the loop.",
                                   addressees=("ChatGPT", "Vinay Raj")) is not None
    assert bh.origin_ack_violation("Thanks for checking Vinay.",
                                   addressees=("Vinay Raj",)) is not None
    assert bh.origin_ack_violation("Vinay, thanks.", addressees=("Vinay Raj",)) is None
    ...
    assert bh.origin_ack_violation("Thanks, <@U-vinay>!",
                                   addressee_ids=("U-vinay",)) is None
    assert bh.origin_ack_violation("thanks <@U-vinay> — got it",
                                   addressee_ids=("U-vinay",)) is None
