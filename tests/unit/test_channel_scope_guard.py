...
async def test_a_bot_requester_cannot_resolve_channel_names_either():
    b = _lookup_bot([{"id": "C_INSIGHTS", "name": "product-insights"}], infos={"C_INSIGHTS": _SHARED})

    res = await b.execute_lookup_channel(_ctx(human=False), {"name": "product-insights"})
...
async def test_lookup_resolves_an_exact_name():
    b = _lookup_bot([{"id": "C_INSIGHTS", "name": "product-insights", "is_private": True}],
                    infos={"C_INSIGHTS": _SHARED})
    ...
    res = await b.execute_lookup_channel(_ctx(is_dm=True), {"name": "product-insights"})

    assert res == {"ok": True, "id": "C_INSIGHTS", "name": "product-insights", "is_private": True}
...
async def test_lookup_normalizes_hash_and_case():
    ...
    assert res["ok"] is True and res["id"] == "C_INSIGHTS"


@pytest.mark.asyncio
async def test_lookup_matches_name_normalized_too():
    b = _lookup_bot([{"id": "C_X", "name_normalized": "product-insights"}],
                    infos={"C_X": _SHARED})
...
async def test_lookup_reports_ambiguity_instead_of_guessing():
    b = _lookup_bot(
        [{"id": "C_ONE", "name": "product-insights"}, {"id": "C_TWO", "name": "product-insights"}],
...
async def test_lookup_requires_bot_membership_via_conversations_info():
    ...
    b = _lookup_bot([{"id": "C_THEIRS", "name": "product-insights"}],
                    infos={"C_THEIRS": _BOT_ABSENT})

    res = await b.execute_lookup_channel(_ctx(), {"name": "product-insights"})
...
async def test_lookup_without_a_requester_is_refused():
    ...
    res = await b.execute_lookup_channel(_ctx(user_id=None), {"name": "product-insights"})
...
async def test_lookup_shares_the_membership_walk_with_the_guard():
    ...
    await b._channel_is_accessible("C_INSIGHTS", ctx)
    await b.execute_lookup_channel(ctx, {"name": "product-insights"})
