...
CACHE = {
    PETER: {"username": "peter", "real_name": "Peter Rotella"},
...
NAME_TO_ID = {"peter": PETER, "Peter Rotella": PETER, "Claude": CLAUDE}


def test_outbound_bracketed_name_resolves():
    assert encode_outbound_mentions("ping <@Peter Rotella> now", NAME_TO_ID) == f"ping <@{PETER}> now"
...
def test_roster_builds_block_with_ids():
    out = build_roster_text({PETER: "Peter Rotella", CLAUDE: "Claude"})
    assert f"<@{PETER}>" in out and f"<@{CLAUDE}>" in out
    assert "Peter Rotella" in out
...
def test_roster_skips_self():
    out = build_roster_text({PETER: "Peter Rotella", BOT: "ChatGPT"}, bot_user_id=BOT)
...
def test_mixin_build_name_to_id_map():
    ...
    assert m["peter"] == PETER and m["Peter Rotella"] == PETER and m["Claude"] == CLAUDE
...
def test_mixin_format_text_encodes_then_converts():
    stub = _FmtStub(user_cache=CACHE)
    assert stub.format_text("ping <@Peter Rotella>") == f"ping <@{PETER}>"
