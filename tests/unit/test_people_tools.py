...
async def test_lookup_by_name_via_db_rows():
    client, api = _client(users_info=_users_info(user_id="U9", display="dana", real="Dana Whitfield"))
    db = MagicMock()
    db.get_all_users_async = AsyncMock(return_value=[
        {"user_id": "U9", "username": "dana", "real_name": "Dana Whitfield"}])
...
async def test_lookup_ambiguous_returns_candidates():
    client, api = _client(user_cache={
        "U1": {"username": "sam", "real_name": "Sam Sutton"},
        "U2": {"username": "sammy", "real_name": "Sam Sutton"}})
    db = MagicMock()
    db.get_all_users_async = AsyncMock(return_value=[])
    res = await execute_lookup_user(_ctx(client, db=db), {"user": "Sam Sutton"})
...
def test_registry_gating_on_enable_people_tools():
    ...
    # Assert about the people tools themselves, not the whole registry: tools registered
    ...
    people_tools = {"lookup_user", "list_channel_members"}
    assert people_tools <= build(True)
    assert people_tools.isdisjoint(build(False))
