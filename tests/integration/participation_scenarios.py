...
HUMANS_ON_VENDORS = _tail(
    '- JS Vachon [human] (top-level): "My first impression: Opus 5 Medium is doing better than '
    '4.8 XHigh in short and long horizon tasks"',
    '- JS Vachon [human] (top-level): "While being cheaper :money-with-wings-gif:"',
...
BOT_WAS_CORRECTED = _tail(
    '- Kousha Mazloumi [human] (top-level): "check your prompts based on the article i shared '
    'yesterday"',
    '- Kousha Mazloumi [human] (top-level): "they want u to remove like 80% of sys prompts now"',
...
SENDER_WITH_SELF = _tail(
    '- Vinay Raj [human] (top-level): "chatgpt, what\'s the retention on the ambient artifacts '
    'table?"',
...
SELF_DID_THE_WORK = _tail(
    '- Kousha Mazloumi [human] (top-level): "chatgpt can you pull the Q3 defect counts by line?"',
    '- ChatGPT [self] (in a thread): "Line A 412, Line B 388, Line C 1,204 — C is the outlier, '
    'almost entirely from the SEA-4 run."',
)
...
OTHERS_WIN = _tail(
    '- Dana Whitfield [human] (top-level): "nightly went from 12 min to 40, no idea why"',
    '- Vinay Raj [human] (top-level": "try polling for replica lag instead of the fixed sleep"',
)

# Mirrors production: a second assistant is a channel member and the roster marks it as one.
PEOPLE = ("~7 members; recently active: JS Vachon, Peter Rotella, Kousha Mazloumi, "
          "Vinay Raj, Dana Whitfield, Claude (assistant)")
...
SCENARIOS = [
    ...
    _s("collective-your", "addressee",
       ...
       real=True, sender_name="Kousha Mazloumi"),
    ...
    _s("talked-about-not-to", "addressee",
       ...
       sender_name="Vinay Raj", name_hit=True),
    _s("third-party-praise-after-rebuff", "addressee",
       ...
       real=True, sender_name="Kousha Mazloumi", name_hit=True,
    ...
    _s("direct-summons", "addressee-self",
       ...
       sender_name="Peter Rotella", name_hit=True),
    _s("name-typo", "addressee-self",
       ...
       sender_name="JS Vachon", name_hit=True),
    _s("bare-you-continues-self", "addressee-self",
       ...
       sender_name="Vinay Raj", channel_addressee_tail=SENDER_WITH_SELF),
    ...
    _s("thread-followup-to-self", "addressee-self",
       ...
       sender_name="Kousha Mazloumi", is_thread_reply=True,
    ...
    _s("correction-of-bot", "relation",
       ...
       real=True, sender_name="Peter Rotella", channel_addressee_tail=BOT_WAS_CORRECTED),
    _s("objection-to-participation", "relation",
       ...
       real=True, sender_name="Peter Rotella", name_hit=True,
       channel_addressee_tail=BOT_WAS_CORRECTED),
    _s("hush", "relation",
    ...
    _s("reinvite", "relation",
       ...
       sender_name="Peter Rotella", name_hit=True, channel_addressee_tail=BOT_WAS_CORRECTED),
    ...
    _s("other-bot-already-acked", "shape",
       ...
       sender_name="Vinay Raj", channel_addressee_tail=OTHER_BOT_ACKED),
    _s("human-closer", "react",
       ...
       sender_name="Kousha Mazloumi", channel_addressee_tail=SELF_DID_THE_WORK),
    ...
    _s("delegation-to-self", "react",
       ...
       sender_name="Vinay Raj", name_hit=True, channel_addressee_tail=SELF_DID_THE_WORK),
    ...
    _s("thanks-to-other-human", "react",
       "thanks Kousha, that's exactly what I needed",
       ("ignore",),
       "CONTROL. A thanks with the same shape as human-closer, addressed to a person.",
       sender_name="Peter Rotella", channel_addressee_tail=SELF_DID_THE_WORK),
    ...
    _s("mentions-only-unaddressed", "strictness",
       ...
       sender_name="Vinay Raj", strictness="mentions_only"),
    _s("mentions-only-summons", "strictness",
       ...
       sender_name="Vinay Raj", strictness="mentions_only", name_hit=True),
    _s("active-open-question", "strictness",
       ...
       "a correctness check.",
       sender_name="JS Vachon", strictness="active"),
]
...
_REBUFF_BY_SENDER = _tail(
    '- Vinay Raj [human] (top-level): "chatgpt what\'s the retention on ambient artifacts?"',
    '- ChatGPT [self] (top-level): "7 days, then the nightly cleanup drops them."',
    '- Vinay Raj [human] (top-level): "that\'s wrong, it\'s 14 — you\'re thinking of thread '
...
_REBUFF_THEN_NEW_ASK = _tail(
    ...
    '- JS Vachon [human] (top-level): "anyway — separate thing"',
)
...
SCENARIOS += [
    _s("rebuff-then-praise-same-sender", "boundary",
       ...
       sender_name="Vinay Raj", channel_addressee_tail=_REBUFF_BY_SENDER),
    _s("rebuff-then-new-request", "boundary",
       "chatgpt can you pull the defect counts for the SEA-4 run?",
       SPEAK,
